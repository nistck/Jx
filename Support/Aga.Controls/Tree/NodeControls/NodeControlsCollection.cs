...
namespace Aga.Controls.Tree.NodeControls
{
	...
	internal class NodeControlCollectionEditor : CollectionEditor
	{
        ...
        {
            string text = base.GetDisplayText(value);
            if (value == null)
                return text;

            Type type = value.GetType();
            string result = string.Format("{0} ({1})", text, type.Name);
            if( value is NodeControl )
            {
                NodeControl nc = value as NodeControl;
                result = string.Format("[{0}] {1}", nc.Column, result);
            }
            return result;
        }
    }
