...
namespace Jx.UI.PGEx
{
    ...
    {				
		...
		protected JxUIFileNameEditor.FileDialogType eDialogType = JxUIFileNameEditor.FileDialogType.LoadFileDialog;
		protected bool bUseFileNameEditor = false;
		
		// Custom choices properties
		protected JxCustomChoices oChoices = null;
		
		// Browsable properties
		protected bool bIsBrowsable = false;
		protected JxBrowsableTypeConverter.LabelStyle eBrowsablePropertyLabel = JxBrowsableTypeConverter.LabelStyle.lsEllipsis;
		...
		protected JxUICustomEventEditor.OnClick MethodDelegate;
		...
		// Custom Editor and Custom kind Converter
		...
		private void BuildAttributes_FilenameEditor()
		{
			...
			JxUIFileNameEditor.FileDialogFilterAttribute FilterAttribute = new JxUIFileNameEditor.FileDialogFilterAttribute(sFilter);
			JxUIFileNameEditor.SaveFileAttribute SaveDialogAttribute = new JxUIFileNameEditor.SaveFileAttribute();
			Attribute[] attrArray;
			attrs.Add(FilterAttribute);
			if (eDialogType == JxUIFileNameEditor.FileDialogType.SaveFileDialog)
			{
		...
		private void BuildAttributes_CustomChoices()
		{
			if (oChoices != null)
			{
				JxCustomChoices.CustomChoicesAttributeList list = new JxCustomChoices.CustomChoicesAttributeList(oChoices.Items);
				ArrayList attrs = new ArrayList();
				Attribute[] attrArray;
				attrs.Add(list);
				attrArray =  (System.Attribute[]) attrs.ToArray(typeof(Attribute));
		...
		private void BuildAttributes_ListboxEditor()
		{
			if (oDatasource != null)
			{
				JxUIListboxEditor.UIListboxDatasource ds = new JxUIListboxEditor.UIListboxDatasource(ref oDatasource);
				JxUIListboxEditor.UIListboxValueMember vm = new JxUIListboxEditor.UIListboxValueMember(sValueMember);
				JxUIListboxEditor.UIListboxDisplayMember dm = new JxUIListboxEditor.UIListboxDisplayMember(sDisplayMember);
				JxUIListboxEditor.UIListboxIsDropDownResizable ddr = null;
				ArrayList attrs = new ArrayList();
				attrs.Add(ds);
				attrs.Add(vm);
				attrs.Add(dm);
				...
				{
					ddr = new JxUIListboxEditor.UIListboxIsDropDownResizable();
					attrs.Add(ddr);
				}
				Attribute[] attrArray;
				attrArray =  (System.Attribute[]) attrs.ToArray(typeof(Attribute));
        ...
        {
            JxBrowsableTypeConverter.BrowsableLabelStyleAttribute style = new JxBrowsableTypeConverter.BrowsableLabelStyleAttribute(eBrowsablePropertyLabel);
            oCustomAttributes = new AttributeCollection(new Attribute[] { style });
        }
		
		private void BuildAttributes_CustomEventProperty()
		{
			JxUICustomEventEditor.DelegateAttribute attr = new JxUICustomEventEditor.DelegateAttribute(MethodDelegate);
			...
			get
			{
				if (bRef)
				{
                    ...
                    else
                    {
                        ...
                        throw new Exception("CustomProperty.value.get");
                    }
				}
				else
				{
			...
			set
			{
				if (bRef)
				{
                    ...
                    else
                    {
                        ...
                        throw new Exception("CustomProperty.value.set");
                    }
				}
				else
				{
        ...
        public JxBrowsableTypeConverter.LabelStyle BrowsableLabelStyle
        {
			get
			{
			...
			set
			{
				...
				if (Update)
				{
					JxBrowsableTypeConverter.BrowsableLabelStyleAttribute style = new JxBrowsableTypeConverter.BrowsableLabelStyleAttribute(value);
					oCustomAttributes = new AttributeCollection(new Attribute[] {style});
				}
			}
		}
        ...
        public JxUIFileNameEditor.FileDialogType FileNameDialogType
        {
			get
			{
				return eDialogType;
        ...
        public JxCustomChoices Choices
        {
			get
			{
				return oChoices;
		...
		public class CustomPropertyDescriptor : PropertyDescriptor
		{
			...
			protected JxCustomProperty oCustomProperty;
			
			public CustomPropertyDescriptor(JxCustomProperty myProperty, Attribute[] attrs) : base(myProperty.Name, attrs)
			{
				if (myProperty == null)
				{
