...
using Jx.UI.Properties;
using Jx.UI.Win32;

namespace Jx.UI.Controls.KRBTabControl
{
    [Designer(typeof(JxTabControlDesigner))]
    public partial class JxTabControl : TabControl
    {
        #region Events
        ...
        private bool m_IsDrawHeader = true; 
        private bool m_IsCaptionVisible = true;
        private bool m_IsDrawEdgeBorder = false;
        private bool m_IsUserInteraction = true;
        private bool m_IsDocumentTabStyle = false;
        private bool m_IsDrawTabSeparator = false; 
        private bool m_HeaderVisibility = false; 
        ...
        private Color m_TabBorderColor = Color.Silver;
        private Color m_TabPageCloseIconColor = Color.Black; 

        // TabPageCloseIconRectangle, CaptionDropDownRectangle, CaptionCloseRectangle  
        private Rectangle m_TabPageCloseIconRectangle;
        private Rectangle m_CaptionDropDownRectangle;
        private Rectangle m_CaptionCloseRectangle; 


        // TabPageCloseButtonState, CaptionDropDownButtonState, CaptionCloseButtonState
        private ButtonState m_TabPageCloseButtonState = ButtonState.Normal;
        private ButtonState m_CaptionDropDownButtonState = ButtonState.Normal;
        private ButtonState m_CaptionCloseButtonState = ButtonState.Normal; 
        ...
        {
            get { return m_TabBorderColor; }
            set
            {
                if (!value.Equals(m_TabBorderColor))
                {
                    m_TabBorderColor = value;
        ...
        {
            get { return m_TabPageCloseIconColor; }
            set 
            {
                if (!value.Equals(m_TabPageCloseIconColor))
                {
                    m_TabPageCloseIconColor = value;
        ...
        public bool HeaderVisibility
        {
            ...
            set
            {
                if (!value.Equals(m_HeaderVisibility))
                {
        ...
        {
            get { return m_IsDrawHeader; }
            set
            {
                if (!value.Equals(m_IsDrawHeader))
                {
                    m_IsDrawHeader = value;
        ...
        public bool IsCaptionVisible
        {
            ...
            set
            {
                if (!value.Equals(m_IsCaptionVisible))
                {
        ...
        {
            get { return m_IsDrawEdgeBorder; }
            set
            {
                if (!value.Equals(m_IsDrawEdgeBorder))
                {
                    m_IsDrawEdgeBorder = value;
                    this.UpdateStyles();
                }
            }
        }
        ...
        {
            get { return m_IsUserInteraction; }
            set 
            {
                if (!value.Equals(m_IsUserInteraction))
        ...
        {
            get { return m_IsDrawTabSeparator; }
            set
            {
                if (!value.Equals(m_IsDrawTabSeparator))
                {
                    m_IsDrawTabSeparator = value;
        ...
        {
            get { return m_IsDocumentTabStyle; }
            set
            {
                if (!value.Equals(m_IsDocumentTabStyle))
                {
                    m_IsDocumentTabStyle = value;
        ...
        public override System.Drawing.Rectangle DisplayRectangle
        {
            get
            {
                ...
                else if (m_IsCaptionVisible && !m_IsDocumentTabStyle && this.TabCount <= 1)
                {
                    switch (_alignments)
                    {
        ...
        protected override void OnDragOver(DragEventArgs drgevent)
        {
            if (!DesignMode)
            {
                if (drgevent.KeyState == 1 && drgevent.Data.GetDataPresent(typeof(JxTabPage)))
                {
                    JxTabPage over = OverTab();
                    if (over != null)
                    {
                        JxTabPage draggingTab = drgevent.Data.GetData(typeof(JxTabPage)) as JxTabPage;

                        int draggingTabIndex = ((JxTabControl)draggingTab.Parent).TabPages.IndexOf(draggingTab);
                        ...
                        if (checkTabs ? overringTabIndex != draggingTabIndex : true)
                        {
                            ...
                            // WidthHit
                            Rectangle rctOver = this.GetTabClientRect(overringTabIndex); //this.GetTabRect(overringTabIndex);
        ...
        protected override void OnDragDrop(DragEventArgs drgevent)
        {
            if (!DesignMode)
            {
                if (drgevent.Data.GetDataPresent(typeof(JxTabPage)))
                {
                    JxTabPage over = OverTab();
                    if (over != null)
                    {
                        ...
                        if (draggingTab != over)
                        {
                            ...
                            {
                                ((JxTabControl)draggingTab.Parent).TabPages.Remove(draggingTab);
        ...
        protected override void OnMouseMove(MouseEventArgs e)
        {
            if (!m_HeaderVisibility)
            {
                ...
                if (m_TabPageCloseIconRectangle.Contains(e.Location))
                {
                    if (processes[0] != 1)
                    {
                        m_TabPageCloseButtonState = ButtonState.Hover;
                ...
                else
                {
                    ...
                    {
                        m_TabPageCloseButtonState = ButtonState.Normal;

                        _mouseImgProcessing[0] = true;
                        Invalidate(m_TabPageCloseIconRectangle);
                        processes[0] = 0;
                    }
                }
                ...
                if (m_IsCaptionVisible)
                {
                    ...
                    if (m_CaptionDropDownRectangle.Contains(e.Location))
                    {
                        if (processes[1] != 1)
                        {
                            m_CaptionDropDownButtonState = ButtonState.Hover;
                    ...
                    else
                    {
                        ...
                        {
                            m_CaptionDropDownButtonState = ButtonState.Normal;
                    ...
                    if (m_CaptionCloseRectangle.Contains(e.Location))
                    {
                        if (processes[2] != 1)
                        {
                            m_CaptionCloseButtonState = ButtonState.Hover;
                            //Console.WriteLine("MouseMove, CaptionCloseButtonState: " + m_CaptionCloseButtonState);
                    ...
                    else
                    {
                        ...
                        {
                            m_CaptionCloseButtonState = ButtonState.Normal;
        ...
        protected override void OnMouseLeave(EventArgs e)
        {
            if (!m_HeaderVisibility)
            {
                if (m_TabPageCloseButtonState == ButtonState.Hover || m_TabPageCloseButtonState == ButtonState.Pressed)
                {
                    processes[0] = 0;
                    m_TabPageCloseButtonState = ButtonState.Normal;
                    Invalidate(m_TabPageCloseIconRectangle);
                }
                else if (m_CaptionDropDownButtonState == ButtonState.Hover || m_CaptionDropDownButtonState == ButtonState.Pressed)
                {
                    processes[1] = 0;
                    m_CaptionDropDownButtonState = ButtonState.Normal;
                    Invalidate(m_CaptionDropDownRectangle);
                }
                else if (m_CaptionCloseButtonState == ButtonState.Hover || m_CaptionCloseButtonState == ButtonState.Pressed)
                {
                    processes[2] = 0;
                    m_CaptionCloseButtonState = ButtonState.Normal;
                    Invalidate(m_CaptionCloseRectangle);
                }
            }
        ...
        protected override void OnGotFocus(EventArgs e)
        {
            ...
            if (!m_HeaderVisibility && m_IsCaptionVisible)
                Invalidate();
        }
        ...
        {
            if (!m_HeaderVisibility && e.Button == MouseButtons.Left)
            {
                if (m_TabPageCloseIconRectangle.Contains(e.Location))
                {
                    m_TabPageCloseButtonState = ButtonState.Pressed;
                    Invalidate(m_TabPageCloseIconRectangle);
                }
                else if (m_IsCaptionVisible && m_CaptionDropDownRectangle.Contains(e.Location))
                {
                    m_CaptionDropDownButtonState = ButtonState.Pressed;
                    Invalidate(m_CaptionDropDownRectangle);
                }
                else if (m_IsCaptionVisible && m_CaptionCloseRectangle.Contains(e.Location))
                {
                    m_CaptionCloseButtonState = ButtonState.Pressed;
                    Invalidate(m_CaptionCloseRectangle);
                }
                else if (!DesignMode && this.TabCount >= 1)
                {
                    // WidthHit
                    Rectangle draggingTab = this.GetTabClientRect(this.SelectedIndex); //this.GetTabRect(this.SelectedIndex);
                    ...
                    if (draggingTab.Contains(e.X, e.Y))
                    {
                        ...
                        this.DoDragDrop((JxTabPage)this.SelectedTab, DragDropEffects.Move);
                    }
                }
            }
        ...
        protected override void OnMouseUp(MouseEventArgs e)
        {
            if (!m_HeaderVisibility && e.Button == MouseButtons.Left)
            {
                ...
                bool isMouseMoved = !pt.Equals(Cursor.Position); 
                
                if ( !isMouseMoved && m_TabPageCloseIconRectangle.Contains(e.Location))
                {
                    m_TabPageCloseButtonState = ButtonState.Hover;
                    Invalidate(m_TabPageCloseIconRectangle);
                    goto Finalize;
                ...
                if (m_IsCaptionVisible)
                {
                    ...
                    if (!isMouseMoved && m_CaptionCloseRectangle.Contains(e.Location))
                    {
                        m_CaptionCloseButtonState = ButtonState.Hover;  
                        Invalidate(m_CaptionCloseRectangle);
                    }
        ...
        {
            //Console.WriteLine("MouseClick: " + e.Location + ", CursorPosition: " + Cursor.Position); 
            if (!m_HeaderVisibility)
            {
                if (e.Button == MouseButtons.Left)
                {
                    ...
                    if (m_IsCaptionVisible)
                    {
                        if (m_CaptionDropDownRectangle.Contains(e.Location))
                        {
                            ...
                            Point dropDownLocation = PointToScreen(new Point(m_CaptionDropDownRectangle.X, m_CaptionDropDownRectangle.Bottom));
                        ...
                        else if (this.TabCount > 0 && m_CaptionCloseRectangle.Contains(e.Location))
                        {
                            JxTabPage closingTabPage = (JxTabPage)this.SelectedTab;
                            ...
                            if (flag)
                            {
                                using (SelectedIndexChangingEventArgs sea = new SelectedIndexChangingEventArgs(closingTabPage, this.SelectedIndex))
                                {
                                    ...
                                    {
                                        //MessageBox.Show("用户取消了操作", Application.ProductName, MessageBoxButtons.OK, MessageBoxIcon.Information);
                                    }
                                    
                                    //Cursor.Position = PointToScreen(e.Location); 
                                    //Console.WriteLine(">>CCR CursorPosition: " + Cursor.Position + ", in CaptionRect? " + m_CaptionCloseRectangle.Contains(Cursor.Position));

                                    if (!m_CaptionCloseRectangle.Contains(Cursor.Position))
                                    {
                                        m_CaptionCloseButtonState = ButtonState.Normal;
                                        Invalidate(m_CaptionCloseRectangle); 
                                    }                                    
                    ...
                    // IsDocumentTabStyle?
                    if (!m_IsDocumentTabStyle ? this.TabCount > 1 : this.TabCount >= 1)
                    {
                        JxTabPage closingTabPage = this.SelectedTab as JxTabPage; 
                        if (closingTabPage != null)
                        {
                            if (m_TabPageCloseIconRectangle.Contains(e.Location))
                            {
                                ...
                                if (!flag)
                                {
                                    using (SelectedIndexChangingEventArgs te = new SelectedIndexChangingEventArgs(closingTabPage, this.SelectedIndex))
                                    {
                                        ...
                                        //Cursor.Position = PointToScreen(e.Location);  
                                        if (!m_TabPageCloseIconRectangle.Contains(Cursor.Position))
                                        {
                                            m_TabPageCloseButtonState = ButtonState.Normal;
                                            Invalidate(m_TabPageCloseIconRectangle);
                                        }
        ...
        {
            //Console.WriteLine("MouseDoubleClick: " + e.Location); 
            if (!m_HeaderVisibility)
            {
                if (e.Button == MouseButtons.Left)
                {
        ...
        protected override void OnControlAdded(ControlEventArgs e)
        {
            ...
            if (e.Control is JxTabPage && e.Control.Text == String.Empty)
            {
                JxTabPage adding = e.Control as JxTabPage; 
                int value = m_pageIndex;
                m_pageIndex++; 

                adding.Name = "选项卡" + value.ToString();
        ...
        protected bool ProcessCmdKey0(ref Message msg, Keys keyData)
        {
            ...
            else
            {
                switch (keyData)
                {
                    ...
                    case Keys.Right:
                        ...
                        break;
                    case Keys.Insert:
                        if (m_IsUserInteraction && MessageBox.Show("是否在当前位置添加一个选项卡?", Application.ProductName, MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                        {
                            JxTabPage tabPage = new JxTabPage();
                            this.Controls.Add(tabPage);
                        }
                        break;
                    case Keys.Delete:
                        if (m_IsUserInteraction && this.TabCount > 0)
                        {
                            if (MessageBox.Show("是否将当前选中的选项卡删除?", Application.ProductName, MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                            {
                                JxTabPage removingTabPage = this.SelectedTab as JxTabPage;
        ...
        {
            foreach (JxTabPage tabPage in this.TabPages)
            {
        ...
        protected override void WndProc(ref Message m)
        {
            ...
            else if (m.Msg == (int)User32.Msgs.WM_LBUTTONDOWN)
            {
                if (this.TabCount > 1)
                {
                    JxTabPage selectingTabPage = OverTab();
                ...
                if (_upDown32 != null && !this.Multiline)
                {
                    if (this.TabCount > 1)
                    {
                        ...
                        for (int i = 0; i < this.TabCount; i++)
                            width += GetTabClientRect(i).Width; //GetTabRect(i).Width;
        ...
        protected virtual JxTabPage OverTab()
        {
            JxTabPage over = null;
            ...
            {
                // WidthHit
                Rectangle currentTabRct = this.GetTabClientRect(currentTabIndex); //this.GetTabRect(currentTabIndex);
                ...
                if (currentTabRct.Contains(pt))
                    over = this.TabPages[currentTabIndex] as JxTabPage;
            }
        ...
        protected virtual bool OnNavigateTabPage(int tabPageIndex, bool wrap)
        {
            ...
            try
            {
                ...
                JxTabPage selectingTabPage = this.TabPages[tabPageIndex] as JxTabPage;
                if (selectingTabPage == null)
                    return true;
                else
                {
        ...
        protected virtual void Draw(Graphics gfx)
        {
            ...
            if (this.SelectedTab != null && !m_HeaderVisibility && (!m_IsDocumentTabStyle ? (m_IsCaptionVisible ? this.TabCount > 1 : true) : true))
            {
                Rectangle rctTabArea = this.DisplayRectangle;

                if (!m_IsCaptionVisible && _tabStyles == TabStyle.VS2010)
                {
        ...
        public Rectangle GetTabClientRect(int nIndex)
        {
            Rectangle rt = this.GetTabRect(nIndex);
            /*
            Image icon = this.GetTabPageIcon(nIndex);
            if (icon != null)
                rt.Width += icon.Width;
            else
                rt.Width += 10;
        ...
        public Rectangle GetTabClientRect0(int nIndex)
        {
            int left = 0; 
            for (int i = 0; i < nIndex; i++)
            {
                Rectangle rt = GetTabRect(i);
                JxTabPage _tabPage = TabPages[i] as JxTabPage;
                if (_tabPage != null && _tabPage.WidthHint > 0 && rt.Width > _tabPage.WidthHint)
                    left = left + _tabPage.WidthHint; 
                else
                    left = left + rt.Width; 
            }
            
            Rectangle currentRt = GetTabRect(nIndex);
            int width = currentRt.Width; 
            JxTabPage page = TabPages[nIndex] as JxTabPage;
            if (page.WidthHint > 0 && width > page.WidthHint)
                width = page.WidthHint;

            Rectangle _rt = new Rectangle(left, currentRt.Top, width, currentRt.Height);
            Console.WriteLine("{0} {1}", nIndex, _rt); 
            return _rt; 
        }
        ...
        {
            JxTabPage page = TabPages[nIndex] as JxTabPage;
            bool pageIsClosable = page != null && page.IsClosable;
            Image _tabIcon = GetTabPageIcon(nIndex);

            // WidthHit
            Rectangle currentTab = GetTabClientRect(nIndex); //this.(nIndex);   
            ...
            if (nIndex == this.SelectedIndex)
            {
                ...
                switch (_alignments)
                {
                    #region Tab Alignment: TOP
                    case TabAlignments.Top: 

                        m_TabPageCloseIconRectangle = new Rectangle()  // Object Initializer
                        {
                        ...
                        using (Pen pen1 = new Pen(m_TabBorderColor))
                        using (GraphicsPath gp = new GraphicsPath())
                        {
                            pen1.DashStyle = borderStyle;
                            if (!m_IsCaptionVisible && m_IsDrawEdgeBorder)
                            {
                            ...
                            if (pageIsClosable)
                            {
                                if (m_TabPageCloseButtonState == ButtonState.Normal)
                                {
                                    using (ImageAttributes attributes = new ImageAttributes())
                                    {
                                        ...
                                        map[1].NewColor = m_TabPageCloseIconColor;

                                        attributes.SetRemapTable(map);
                                        gfx.DrawImage(closeImg, m_TabPageCloseIconRectangle, 0, 0, closeImg.Width, closeImg.Height, GraphicsUnit.Pixel, attributes);
                                    }
                                }
                                else
                                    gfx.DrawImageUnscaled(closeImg, m_TabPageCloseIconRectangle);
                            }
            ...
            else
            {
                if (m_IsDrawTabSeparator)
                {
                    ...
                    using (Custom3DBorder tabBorder = new Custom3DBorder(gfx))
                    {
                        ...
                        {
                            if (_alignments == TabAlignments.Top)
                                tabBorder.Draw3DLine(currentTab.Right, currentTab.Top + 3, currentTab.Right, currentTab.Bottom - 7, ThreeDStyle.Groove, 1);
                            else
                                tabBorder.Draw3DLine(currentTab.Right, currentTab.Bottom - 5, currentTab.Right, currentTab.Top + 9, ThreeDStyle.Groove, 1);
                        }
                        else if (nIndex < this.SelectedIndex && nIndex != 0)
                        {
                            if (_alignments == TabAlignments.Top)
                                tabBorder.Draw3DLine(currentTab.Left, currentTab.Top + 3, currentTab.Left, currentTab.Bottom - 7, ThreeDStyle.Groove, 1);
                            else
                                tabBorder.Draw3DLine(currentTab.Left, currentTab.Bottom - 5, currentTab.Left, currentTab.Top + 9, ThreeDStyle.Groove, 1);
                        }
                    }
                }
            }
            ...
            { 
                ...
                else
                {
                    ...
                    {
                        X = currentTab.X + 5,
                        Y = currentTab.Bottom - img.Height - 2, // - (img.Height + 6),
                        Width = img.Width,
                        Height = img.Height
                    };
            ...
            {
                using (StringFormat format = new StringFormat(StringFormatFlags.LineLimit | StringFormatFlags.NoWrap))
                {
                    ...
                    {
                        if( page.IsClosable )
                            currentTab.Width -= m_TabPageCloseIconRectangle.Width;
                        format.Alignment = StringAlignment.Near;
                    }
                    else
                    {
                        if (m_IsDrawTabSeparator || nIndex == 0)
                        {
                            //currentTab.X -= 5;
                            currentTab.Width += 5; 
        ...
        private Image GetTabPageIcon(int nIndex)
        {
            TabPage tabPage = this.TabPages[nIndex];
            JxTabPage tbp = tabPage as JxTabPage; 

            if (this.ImageList == null || nIndex < 0 || nIndex > this.TabPages.Count - 1)
                return tbp == null ? null : tbp.Icon;

            ImageList.ImageCollection imageCol = this.ImageList.Images; 

            int tabImageIndex = tabPage.ImageIndex;
            if (tabImageIndex != -1 && tabImageIndex >= 0 && tabImageIndex <= imageCol.Count - 1)
                return imageCol[tabImageIndex]; 

            String tabImageKey = tabPage.ImageKey;
            if (imageCol.ContainsKey(tabImageKey))
                return imageCol[tabImageKey];
        ...
        protected virtual void DrawBorder(Graphics gfx)
        {
            ...
            if (m_IsDrawHeader || m_HeaderVisibility || this.TabCount == 0
                || (m_IsCaptionVisible && this.TabCount == 1 && !m_IsDocumentTabStyle))
            {
                ...
                ControlPaint.DrawBorder(gfx, rct, m_BorderColor, borderStyle);
            }
            else
            {
                Rectangle rctTabArea = this.DisplayRectangle;

                using (Pen pen = new Pen(m_BorderColor), pen2 = new Pen(m_TabBorderColor))
                {
                    DashStyle borderStyle;
                    switch (_borderStyle)
                    {
        ...
        public void InvalidateCaptionRect()
        {
            Rectangle rct = GetCaptionRect();
        ...
        protected virtual void DrawCaption(Graphics gfx)
        {
            ...
            if (!m_HeaderVisibility && m_IsCaptionVisible)
            {
                ...
                if (captionRect.Width <= 0 || (this.TabCount == 0 ? this.Height <= 17 : DisplayRectangle.Height <= -6))
                    return;
                ...
                using (Bitmap overlay = new Bitmap(captionRect.Width + 1, captionRect.Height + 1))
                {
                    // Make an associated Graphics object.
                    using (Graphics gr = Graphics.FromImage(overlay))
                    {
                    ...
                    using (ImageAttributes attributes = new ImageAttributes())
                    {
                        ...
                        gfx.DrawImage(overlay, captionRect, 1, 1, overlay.Width - 1, overlay.Height - 1, GraphicsUnit.Pixel, attributes);
                    }
                }

                // Draw Caption Line
                using (Pen captionBorderPen = new Pen(m_BorderColor))
                {
                    DashStyle borderStyle;
                    switch (_borderStyle)
                    {
                    ...
                    gfx.DrawLine(captionBorderPen, captionRect.Left, _alignments == TabAlignments.Top ? captionRect.Y - 1 : captionRect.Bottom, captionRect.Right - 1, _alignments == TabAlignments.Top ? captionRect.Y - 1 : captionRect.Bottom);
                }
                ...
                using (ImageAttributes attributes = new ImageAttributes())
                {
                    ...
                    int iconPos = captionRect.Y + captionRect.Height / 2 - 1;
                    JxTabPage _tabPageEx = this.SelectedTab as JxTabPage;
                    bool _closable = _tabPageEx != null && _tabPageEx.IsClosable;
                    if ((this.SelectedTab != null && !_closable) || this.TabCount <= 0)
                    {
                        m_CaptionDropDownRectangle = new Rectangle(captionRect.Right - (3 + captionDropDownImg.Width), iconPos - captionDropDownImg.Height / 2, captionDropDownImg.Width, captionDropDownImg.Height);
                        if (m_CaptionDropDownButtonState == ButtonState.Hover)
                        {
                            using (Brush buttonBrush = new SolidBrush(this.Focused || this.ContainsFocus ? Color.DarkBlue : Color.White))
                                gfx.FillRectangle(buttonBrush, m_CaptionDropDownRectangle);
                        }
                        else if (m_CaptionDropDownButtonState == ButtonState.Pressed)
                        {
                            using (Brush buttonBrush = new LinearGradientBrush(m_CaptionDropDownRectangle, Color.DarkBlue, Color.LightBlue, LinearGradientMode.Vertical))
                                gfx.FillRectangle(buttonBrush, m_CaptionDropDownRectangle);
                        }
                        gfx.DrawImage(captionDropDownImg, m_CaptionDropDownRectangle, 0, 0, captionDropDownImg.Width, captionDropDownImg.Height, GraphicsUnit.Pixel, attributes);
                    }
                    else
                    {
                        m_CaptionCloseRectangle = new Rectangle(captionRect.Right - (3 + captionCloseImg.Width), iconPos - captionCloseImg.Height / 2, captionCloseImg.Width, captionCloseImg.Height);
                        m_CaptionDropDownRectangle = new Rectangle(m_CaptionCloseRectangle.Left - (3 + captionDropDownImg.Width), iconPos - captionDropDownImg.Height / 2, captionDropDownImg.Width, captionDropDownImg.Height);

                        //Console.WriteLine("CaptionCloseButtonState: " + m_CaptionCloseButtonState); 
                        if (m_CaptionCloseButtonState == ButtonState.Hover)
                        {
                            using (Brush buttonBrush = new SolidBrush(this.Focused || this.ContainsFocus ? Color.DarkBlue : Color.White))
                                gfx.FillRectangle(buttonBrush, m_CaptionCloseRectangle);
                        }
                        else if (m_CaptionCloseButtonState == ButtonState.Pressed)
                        {
                            using (Brush buttonBrush = new LinearGradientBrush(m_CaptionCloseRectangle, Color.DarkBlue, Color.LightBlue, LinearGradientMode.Vertical))
                                gfx.FillRectangle(buttonBrush, m_CaptionCloseRectangle);
                        }
                        ...
                        gfx.DrawImage(captionCloseImg, m_CaptionCloseRectangle, 0, 0, captionCloseImg.Width, captionCloseImg.Height, GraphicsUnit.Pixel, attributes);
                        gfx.DrawImage(captionDropDownImg, m_CaptionDropDownRectangle, 0, 0, captionDropDownImg.Width, captionDropDownImg.Height, GraphicsUnit.Pixel, attributes);
                    }
                }

                // Draw String
                captionRect.Width = m_CaptionDropDownRectangle.Left;
                captionRect.Inflate(-3, -2);

                // Caption Icon
                Image infoIcon = GetSelectedTabPageInformationIcon();
                if (infoIcon != null)
                {
                    int _minSize = captionRect.Width > captionRect.Height? captionRect.Height : captionRect.Width;
                    Rectangle infoIconRect = new Rectangle(0, 0, _minSize, _minSize);
                    Bitmap infoBitmap = new Bitmap(infoIconRect.Width, infoIconRect.Height);
                    Graphics infoGfx = Graphics.FromImage(infoBitmap);
                    infoGfx.SmoothingMode = SmoothingMode.HighQuality; 
                    infoGfx.DrawImage(infoIcon, infoIconRect);

                    gfx.DrawImage(infoBitmap, captionRect.Left, captionRect.Top, infoIconRect.Width, infoIconRect.Height);

                    captionRect.Offset(infoIconRect.Width + 2, 0); 
                }                
                ...
                if (captionRect.Width > 0)
                {
                    using (Font captionFont = new System.Drawing.Font(this.Font.Name, this.Font.Size > 12 ? 12 : this.Font.Size, this.Focused || this.ContainsFocus ? _gradientCaption.ActiveCaptionFontStyle : FontStyle.Regular, GraphicsUnit.Pixel))
                    {
                        ...
                        using (StringFormat captionTextFormat = new StringFormat(StringFormatFlags.LineLimit))
                        {
                            ...
                            {
                                text = this.SelectedTab.Text; 
                                JxTabPage page = this.SelectedTab as JxTabPage;
                                if (page != null)
                                    text = page.Information;
                            }

                            gfx.DrawString(isContainsTabPages ? text : "Tab container is empty, drag or add a new tab page here!", captionFont,
                                captionTextBrush, captionRect, captionTextFormat);
                        }
                    }
                }
            }
        ...
        private Image GetSelectedTabPageInformationIcon()
        {
            ...
            ImageList.ImageCollection imageCol = this.ImageList.Images;
            int imageIndex = page.InformationImageIndex;
            if (imageIndex >= 0 && imageIndex < imageCol.Count)
                return imageCol[imageIndex]; 

            String imageKey = page.InformationImageKey;
            if (imageCol.ContainsKey(imageKey))
                return imageCol[imageKey]; 
        ...
        protected virtual void OnContextMenuShown(ContextMenuShownEventArgs e)
        {
            ...
            if (this.TabCount > 0)
            {
                menuItem = new ToolStripMenuItem("关闭当前选项卡", Resources.delete_12x12, null, Keys.Control | Keys.C);                
                ...
                if (this.TabCount > 1)
                {
                    menuItem = new ToolStripMenuItem("关闭其他选项卡", Resources.PushpinHS, null, Keys.Control | Keys.A);
                    menuItem.Click += (sender, ea) =>
                    {
                        // Iterate All TabPages in the collection.
                        for (int i = this.TabCount - 1; i >= 0; i--)
                        {
                            JxTabPage currentTabPage = this.TabPages[i] as JxTabPage;
            ...
            foreach (JxTabPage tab in this.TabPages)
            {
                ...
                menuItem.Click += (sender, ea) =>
                    {
                        this.HideTab((JxTabPage)this.TabPages[((ToolStripItem)sender).Name]);
                    };
            ...
            {
                foreach (JxTabPage tab in this._tabPageExPool)
                {
                    menuItem = new ToolStripMenuItem(tab.ToString(), this.ImageList != null ? (tab.ImageIndex != -1 ? this.ImageList.Images[tab.ImageIndex] : null) : null, null, tab.Name);
                    menuItem.Click += (sender, ea) =>
                    {
            ...
            if (availableMenuItems.Count > 0)
            {
                ...
                String text = String.Format("所有选项卡 ({0}个)", availableMenuItems.Count); 
                menuItem = new ToolStripMenuItem(text, Resources.InsertTabControlHS, availableMenuItems.ToArray());
                e.ContextMenu.Items.Add(menuItem);
        ...
        private void SelectNextAvailableTabPage()
        {
            ...
            using (SelectedIndexChangingEventArgs e = new SelectedIndexChangingEventArgs((JxTabPage)SelectedTab, 0))
            {
                // Fire a Notification Event.
                OnSelectedIndexChanging(e);
        ...
        private int ScrollPosition()
        {
            ...
            {
                // WidthHit
                tabRect = GetTabClientRect(value + 1);// GetTabRect(value + 1);
                value++;
        ...
        private void ScrollRight(object sender, EventArgs e)
        {
            ...
            // WidthHit
            if (GetTabClientRect(this.TabCount - 1).Right <= this._myScroller.Left)
            {
                _myScroller._rightScroller.Enabled = false;
                return;
            }
        ...
        {
            Bitmap bmp;
            switch (m_TabPageCloseButtonState)
            {
        ...
        public void HideTab(JxTabPage TabPage)
        {
            if (_tabPageExPool == null)
        ...
        public void ShowTab(JxTabPage TabPage)
        {
            if (_tabPageExPool != null)
            {
        ...
        class ButtonsCaptionEditor : UITypeEditor
        {
            ...
            public override void PaintValue(PaintValueEventArgs e)
            {
                if (e.Context.Instance is JxTabControl)
                {
                    JxTabControl parent = e.Context.Instance as JxTabControl;
        ...
        class TabpageExCollectionEditor : CollectionEditor
        {
            ...
            protected override void DestroyInstance(object instance)
            {
                ...
                ISelectionService selectionService = (ISelectionService)GetService(typeof(ISelectionService));
                selectionService.SetSelectedComponents(new IComponent[] { (JxTabControl)Context.Instance }, SelectionTypes.Auto);
            }
        ...
        class TabPageExPool : System.Collections.CollectionBase, IDisposable
        {
            ...
            public JxTabPage this[int index]
            {
                get
                {
                    return (JxTabPage)this.List[index];
                }
                set
                {
            ...
            public JxTabPage this[string name]
            {
                get
                {
                    foreach (JxTabPage tabPage in this)
                    {
            ...
            public void Add(JxTabPage TabPage)
            {
            ...
            public void Remove(JxTabPage TabPage)
            {
                if (this.List.Contains(TabPage))
                    this.List.Remove(TabPage);
            }

            public bool Contains(JxTabPage TabPage)
            {
                if (this.List.Contains(TabPage))
                    return true;
                else
            ...
            public int IndexOf(JxTabPage TabPage)
            {
    ...
    public class JxTabControlDesigner : System.Windows.Forms.Design.ParentControlDesigner
    {
        #region Instance Members
        ...
        public override DesignerActionListCollection ActionLists
        {
            get
            {
                if (_actionLists == null)
                {
                    ...
                    _actionLists.Add(new JxTabControlActionList((JxTabControl)Control));
                }
        ...
        public override DesignerVerbCollection Verbs
        {
            get
            {
                if (_verbs == null)
                {
                    ...
                    JxTabControl parentControl = Control as JxTabControl;
        ...
        protected override void PreFilterProperties(System.Collections.IDictionary properties)
        {
            ...
            properties["Location"] = TypeDescriptor.CreateProperty(typeof(JxTabControl),
              (PropertyDescriptor)properties["Location"], BrowsableAttribute.No);
            properties["ShowToolTips"] = TypeDescriptor.CreateProperty(typeof(JxTabControl),
            ...
            if (parentControl != null)
            {
                ...
                {
                    properties["ItemSize"] = TypeDescriptor.CreateProperty(typeof(JxTabControl),
                      (PropertyDescriptor)properties["ItemSize"], BrowsableAttribute.No);
                    properties["TabStyles"] = TypeDescriptor.CreateProperty(typeof(JxTabControl),
                      (PropertyDescriptor)properties["TabStyles"], BrowsableAttribute.No);
                    properties["Alignments"] = TypeDescriptor.CreateProperty(typeof(JxTabControl),
                      (PropertyDescriptor)properties["Alignments"], BrowsableAttribute.No);
                    properties["UpDownStyle"] = TypeDescriptor.CreateProperty(typeof(JxTabControl),
                      (PropertyDescriptor)properties["UpDownStyle"], BrowsableAttribute.No);
                    properties["HeaderStyle"] = TypeDescriptor.CreateProperty(typeof(JxTabControl),
                      (PropertyDescriptor)properties["HeaderStyle"], BrowsableAttribute.No);
                    properties["TabGradient"] = TypeDescriptor.CreateProperty(typeof(JxTabControl),
                      (PropertyDescriptor)properties["TabGradient"], BrowsableAttribute.No);
                    properties["IsDrawHeader"] = TypeDescriptor.CreateProperty(typeof(JxTabControl),
                      (PropertyDescriptor)properties["IsDrawHeader"], BrowsableAttribute.No);
                    properties["CaptionButtons"] = TypeDescriptor.CreateProperty(typeof(JxTabControl),
                      (PropertyDescriptor)properties["CaptionButtons"], BrowsableAttribute.No);
                    properties["TabBorderColor"] = TypeDescriptor.CreateProperty(typeof(JxTabControl),
                      (PropertyDescriptor)properties["TabBorderColor"], BrowsableAttribute.No);
                    properties["GradientCaption"] = TypeDescriptor.CreateProperty(typeof(JxTabControl),
                      (PropertyDescriptor)properties["GradientCaption"], BrowsableAttribute.No);
                    properties["BackgroundColor"] = TypeDescriptor.CreateProperty(typeof(JxTabControl),
                      (PropertyDescriptor)properties["BackgroundColor"], BrowsableAttribute.No);
                    properties["BackgroundImage"] = TypeDescriptor.CreateProperty(typeof(JxTabControl),
                      (PropertyDescriptor)properties["BackgroundImage"], BrowsableAttribute.No);
                    properties["BackgroundHatcher"] = TypeDescriptor.CreateProperty(typeof(JxTabControl),
                      (PropertyDescriptor)properties["BackgroundHatcher"], BrowsableAttribute.No);
                    properties["CaptionRandomizer"] = TypeDescriptor.CreateProperty(typeof(JxTabControl),
                      (PropertyDescriptor)properties["CaptionRandomizer"], BrowsableAttribute.No);
                    properties["IsDrawTabSeparator"] = TypeDescriptor.CreateProperty(typeof(JxTabControl),
                ...
                if (parentControl.IsDrawHeader)
                {
                    ...
                    {
                        case JxTabControl.TabHeaderStyle.Hatch:
                            properties["BackgroundColor"] = TypeDescriptor.CreateProperty(typeof(JxTabControl),
                              (PropertyDescriptor)properties["BackgroundColor"], BrowsableAttribute.No);
                            properties["BackgroundImage"] = TypeDescriptor.CreateProperty(typeof(JxTabControl),
                              (PropertyDescriptor)properties["BackgroundImage"], BrowsableAttribute.No);
                            break;
                        case JxTabControl.TabHeaderStyle.Solid:
                            properties["BackgroundImage"] = TypeDescriptor.CreateProperty(typeof(JxTabControl),
                              (PropertyDescriptor)properties["BackgroundImage"], BrowsableAttribute.No);
                            properties["BackgroundHatcher"] = TypeDescriptor.CreateProperty(typeof(JxTabControl),
                              (PropertyDescriptor)properties["BackgroundHatcher"], BrowsableAttribute.No);
        ...
        private void OnAddTab(Object sender, EventArgs e)
        {
            ...
            JxTabPage newTab = (JxTabPage)_designerHost.CreateComponent(typeof(JxTabPage));
        ...
        {
            JxTabControl parentControl = e.Component as JxTabControl;
    ...
    public class JxTabControlActionList : DesignerActionList
    {
        #region Instance Members

        private JxTabControl _linkedControl;
        ...
        public JxTabControlActionList(JxTabControl control)
            : base(control)
        {
        ...
        public JxTabControl.TabStyle TabStyles
        {
        ...
        public JxTabControl.TabAlignments Alignments
        {
        ...
        public Color FirstColor
        {
            ...
            set
            {
                ...
                {
                    JxTabControl.GradientTab oldGradient = _linkedControl.TabGradient;
        ...
        public void AddTab()
        {
            ...
            try
            {
                ...
                JxTabPage newTab = (JxTabPage)_host.CreateComponent(typeof(JxTabPage));
                newTab.Text = newTab.Name;
    ...
    public class JxTabPage : TabPage
    {
        ...
        private int _widthHit = 0; 
        private string _information = null;
        private int _informationImageIndex = -1;
        private String _informationImageKey = null;
        ...
        [Description("Tab Width Hit")]
        [DefaultValue(0)]
        [Browsable(true)]
        public int WidthHint
        {
            get { return _widthHit; }
            set 
            {
                int OldValue = _widthHit; 
                _widthHit = value < 0 ? 0 : value;
        ...
        public new string Text
        {
            ...
            {                
                ...
                { 
                    ...
                    //base.Text = base.Text.PadRight(base.Text.Length + 6);
                    _text = base.Text;
                }
            }
        }

        [Description("Information shown in caption erea")]
        [Browsable(true)]
        public string Information
        {
            ...
            set
            {
                if (value != null && !value.Equals(_information))
                {
                    String _oldInfo = _information; 
                    String _info = value.Trim();
                    _information = _info.PadRight(_info.Length + 2).TrimEnd();

                    if( _oldInfo != null && !_oldInfo.Equals(_information) ) 
                        _UpdateCaption(); 
                }
            }
        }
        ...
        public int InformationImageIndex
        {
            get { return _informationImageIndex; }
            set
            {
                int _infoImageIndex = _informationImageIndex; 
                _informationImageIndex = value;
                _informationImageKey = "";

                JxTabControl tc = this.Parent as JxTabControl;
                if ( tc != null && tc.ImageList != null && _infoImageIndex != _informationImageIndex)
                {
                    _UpdateCaption(); 
                }
            }
        ...
        [Localizable(true)]
        [Editor("System.Windows.Forms.Design.ImageIndexEditor, System.Design, Version=4.0.0.0, Culture=neutral, PublicKeyToken=b03f5f7f11d50a3a", typeof(UITypeEditor))]
        public String InformationImageKey
        {
            get { return _informationImageKey; }
            set
            {
                String _infoImageKey = _informationImageKey; 
                _informationImageKey = value;
                _informationImageIndex = -1;

                JxTabControl tc = this.Parent as JxTabControl;
                if (tc != null && tc.ImageList != null && !String.Equals(_infoImageKey, _informationImageKey))
                {
                    _UpdateCaption();
                }
            }
        ...
        private void _NotifyParent()
        {
            JxTabControl TC = this.Parent as JxTabControl;
            if (TC == null)
                return;

            Rectangle rect = TC.GetTabClientRect(Index);
            _NotifyParent(rect); 
        }
         
        private void _NotifyParent(Rectangle rect)
        {
            if (this.Parent == null)
                return;
            if (rect != null)
        ...
        private void _UpdateCaption()
        {
            JxTabControl tc = this.Parent as JxTabControl;
            if (tc != null && tc.SelectedTab.Equals(this))
                tc.InvalidateCaptionRect();
        }
